...
train_data = np.zeros((155,997,26))
train_label = np.zeros((155))
count=0
#for j in range (1,4):
for i in range (1,97): 
...
for i in range (1,43):
    (rate,sig) = wav.read("E:/Gauru/Project2k18/L2/%d.wav"%(i))
    #small changes are not considered 
    mfcc_feat = mfcc(sig,rate,nfft=1104)
    #humans cant percieve loudness in linear scale but in log
    train_data[count,:,:] = np.mean(logfbank(sig,rate),axis=0)
...
for i in range (1,18):
    (rate,sig) = wav.read("E:/Gauru/Project2k18/L3/%d.wav"%(i))
    #small changes are not considered 
    mfcc_feat = mfcc(sig,rate,nfft=1104)
    #humans cant percieve loudness in linear scale but in log
    train_data[count,:,:] = np.mean(logfbank(sig,rate),axis=0)
